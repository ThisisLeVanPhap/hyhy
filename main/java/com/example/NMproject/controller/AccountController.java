...
public class AccountController {
	...
	public ResponseEntity<?> login(@RequestBody RegisterRequest request, HttpSession session) {
		...
		return user.map(u -> {
			...
			return ResponseEntity.ok(Map.of("message", "User login successfully", "userID", u.getId(), "username",
...
//	}
	...
	public ResponseEntity<?> register(@RequestBody RegisterRequest request) {
		try {
			...
			// Trả về phản hồi thành công mà không cần AccountResponse
			Map<String, Object> response = new HashMap<>();
			response.put("userID", newUser.getId());
			response.put("username", newUser.getUsername());
			response.put("email", newUser.getEmail()); // Nếu bạn muốn trả về email
			response.put("pathPicture", "/default/path.jpg"); // Nếu bạn có trường pathPicture
